...
class ROI_Attention(nn.Module):
    def __init__(
        ...
        Args:
            ...
            window_size (tuple): The size of the attention window. Defaults to (2, 4, 4).
    ...
    def forward(self, q_inp, k_inp, flow_f):
        """
        Args:
        ...
        def apply_flow_alignment(flow, keyframe):
            ...
            return F.grid_sample(keyframe.float(), vgrid_scaled, mode='bilinear', padding_mode='border')
        ...
        k_r = k_inp[:, 1]
        k_inp = torch.cat([k_f.unsqueeze(1), k_r.unsqueeze(1)], dim=1)  # 直接拼接而非 stack        
        #k_inp = torch.stack([k_f, k_r], dim=1)

        # Norm and convolution with mixed precision
        ...
        # q = rearrange(q, 'b c (h p1) (w p2) -> b (h w) c', p1=hb, p2=wb)  # Query: [b * f_q, hw, c]
        # k = rearrange(k, 'b f c (h p1) (w p2) -> b (h w) (f c)', p1=hb, p2=wb)  # Key: [b * f_k, hw, f * c]
        # v = rearrange(v, 'b f c (h p1) (w p2) -> b (h w) (f c)', p1=hb, p2=wb)  # Value: [b * f_k, hw, f * c]
        ...
        #out = rearrange(out, 'b (h w) d -> b d h w', h=h // hb, w=w // wb) 
...
class ROI_Block(nn.Module):
    def __init__(self,
        ...
        Args:
            ...
            window_size (tuple): Window size for ROI_Attention. Defaults to (2, 4, 4).
        """
        super().__init__()
        self.patch_embed = patch_embed  
        self.attn = ROI_Attention(embed_dims=embed_dims, window_size=window_size) 

    def forward(self, x, flows_forward, num_frames, n):
        """
        Args:
        ...
        with torch.cuda.amp.autocast():  # 混合精度计算        
            for i in range(n):
                ...
                segment = x[start_idx:end_idx] # m + 2 frames per segment
            
                # Extract frames for the flow(m+1)
                flows_f_segment = flows_forward[:, start_idx:end_idx - 1]# flows_forward：frame0 -> frame1, ..., framem -> framem+1
                #flows_b_segment = flows_backward[:, start_idx + 1:end_idx] #flows_backward：frame1 -> frame0, ..., framem+1 -> framem

                for j in range(1, middle_frames_per_segment + 1):  # Process middle m frames
                    flow_f = flows_f_segment[:, j]
                    #flow_b = flows_b_segment[:, j]
                    
                    # Query and key frames (memory-efficient slicing and permute)
                    q_inp = segment[j:j + 1].contiguous().transpose(0, 1)  # Shape: [batch_size, 1, channels, height, width]
                    k_inp = torch.cat([segment[j - 1:j], segment[j:j + 1], segment[j + 1:j + 2]], dim=0)  # 前一帧、中间帧、后一帧
                    k_inp = k_inp.contiguous().transpose(0, 1)  # Shape: [batch_size, 3, channels, height, width]               

                    # Call ROI_Attention
                    mt = self.attn(q_inp=q_inp, k_inp=k_inp, flow_f=flow_f)  # [batch_size, embed_dims, height, width]
                    outs.append(mt)
        ...
        outs = outs.permute(0, 2, 1, 3, 4) # BCTHW
...
class Mask_Module4TimeSformer_ada(BaseModule):
    ...
    def compute_flow(self, frames):
        ...
        flows_forward = []
        scale_factor = 0.5  # 多尺度缩放因子
        
        with torch.cuda.amp.autocast():  # 混合精度计算，减少显存占用
            for i in range(t - 1):
                # 低分辨率光流计算
                frame1_low = F.interpolate(frames[:, :, i], scale_factor=scale_factor, mode='bilinear', align_corners=False)
                frame2_low = F.interpolate(frames[:, :, i + 1], scale_factor=scale_factor, mode='bilinear', align_corners=False)
                flow_low = self.spynet(frame2_low, frame1_low)  # SPyNet 光流计算

                # 上采样回高分辨率
                flow_high = F.interpolate(flow_low, size=(h, w), mode='bilinear', align_corners=False)

                # 逐步释放中间张量
                del frame1_low, frame2_low, flow_low
                torch.cuda.empty_cache()
                
                flows_forward.append(flow_high)

        # 将光流列表堆叠成张量
        flows_forward = torch.stack(flows_forward, dim=1).contiguous()   # [batch_size, t-1, h, w]
    ...
    def forward(self, frames, num_frames, n):
        ...
        flows_forward = self.compute_flow(frames)

        # Call ROI_Block with contiguous tensors to ensure memory efficiency
        frames = frames.contiguous()
        flows_forward = flows_forward.contiguous()

        # Extract motion probability distributions using ROI_Block
        mt = self.roi_block(frames, flows_forward, num_frames, n)  # [batch_size * num_frames, num_patches, embed_dims]
...
class TimeSformer_ada(nn.Module):
    ...
    def __init__(self,
                 ...
                 mask_module={}, 
                 gate_layers=[],  # 通过 gate_layers 配置启用门控机制的层
                 **kwargs):
        ...
        if transformer_layers is None:
            ...
            if self.attention_type == 'divided_space_time':
                ...
                for i in range(num_transformer_layers):
                    # 判断当前层是否在 gate_layers 中
                    enable_gate = i in gate_layers  # 如果在 gate_layers 中，则启用门控

                    # 对于第0层，使用 DividedTemporalAttentionWithNorm_ada
                    if i == 0:
                        attn_type = 'DividedTemporalAttentionWithNorm_ada'  # 引入ROI信息
                    # 如果启用门控，则使用 DividedTemporalAttentionWithNorm_ada2
                    elif enable_gate:
                        attn_type = 'DividedTemporalAttentionWithNorm_ada2'  # 启用门控机制的时序注意力模块
                    else:
                        attn_type = 'DividedTemporalAttentionWithNorm'  # 默认的时序注意力模块

                    _transformerlayers_cfg.append(dict(
                        type='BaseTransformerLayer',
                        attn_cfgs=[
                            dict(
                                type=attn_type,  # 根据 gate_layers 配置选择时序注意力模块
                                embed_dims=embed_dims,
                                num_heads=num_heads,
    ...
    def forward(self, x):
        ...
        # 保留中间 m 帧
        segment_length = (x.shape[2] - num_frames) // n + num_frames // n
        x_segments = []
        for i in range(n):
        ...
        x = torch.cat(x_segments, dim=2)  # [batch_size, channels, num_frames, height, width]
        ...
        x = self.transformer_layers(x, None, None, motion_mask=motion_mask)
