...
class ROI_Attention(nn.Module):
    def __init__(
        ...
        self.to_kv = nn.Conv3d(dim, inner_dim * 2, kernel_size=(3, 1, 1), bias=False)
    ...
    def forward(self, q_inp, k_inp, flow):
        ...
        # Keyframe retrieval
        k_f, k_r, k_b = k_inp[:, 0], k_inp[:, 1], k_inp[:, 2]  # Middle frames + first/last frames as references
        ...
        q_inp_t = q_inp.permute(0, 2, 1, 3, 4) # b, t, c, h, w => b, c, t, h, w
        k_inp_t = k_inp.permute(0, 2, 1, 3, 4) # b, t, c, h, w => b, c, t, h, w
        q = self.to_q(q_inp_t)
        k, v = self.to_kv(k_inp_t).chunk(2, dim=1)

        # Rearrange tensors for attention computation
        q, k, v = map(lambda t: rearrange(t, 'b c t (h p1) (w p2) -> (b h w) (t p1 p2) c', b=b, p1=hb, p2=wb), (q, k, v))
        ...
        out = out.view(b, f_q, -1, h, w).permute(0, 2, 1, 3, 4) # b, t, c, h, w => b, c, t, h, w
        out = self.to_out(out)
